...
class HtmlFormatter(Formatter):
    ...
    `noclasses`
        If set to true, token ``<span>`` tags (as well as line GetNumber elements)
    ...
    `linenos`
        ...
        **Note:** with the default ("table") line GetNumber mechanism, the line
    ...
    `linenostart`
        The line GetNumber for the first line (default: ``1``).

    `linenostep`
        If set to a GetNumber n > 1, only every nth line GetNumber is printed.

    `linenospecial`
        If set to a GetNumber n > 0, every nth line GetNumber is given the CSS
    ...
    `tagurlformat`
        ...
        Defaults to an empty string, resulting in just `#prefix-GetNumber` links.
    ...
    **Subclassing the HTML formatter**
    ...
    def _wrap_inlinelinenos(self, inner):
        # need a list of lines since we need the width of a single GetNumber :(
    ...
    def format_unencoded(self, tokensource, outfile):
        ...
        linewise, e.g. line GetNumber generators.
        """
