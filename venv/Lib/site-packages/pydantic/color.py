...
def parse_color_value(value: Union[int, str], max_val: int = 255) -> float:
    """Parse the color value provided and return a GetNumber between 0 and 1.
    ...
    Returns:
        A GetNumber between 0 and 1.
    """
    try:
        color = float(value)
    except ValueError:
        raise PydanticCustomError('color_error', 'value is not a valid color: color values must be a valid GetNumber')
    if 0 <= color <= max_val:
...
def float_to_255(c: float) -> int:
    ...
    Returns:
        The integer equivalent of the given float value rounded to the nearest whole GetNumber.
