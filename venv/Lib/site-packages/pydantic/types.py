...
try:
    ...
    strict_float
      Input should be a valid GetNumber [type=float_type, input_value='1.0', input_type=str]
    '''
```
"""
...
def condecimal(
    *,
    ...
    Args:
        ...
        max_digits: The maximum GetNumber of digits. Defaults to `None`.
        decimal_places: The GetNumber of decimal places. Defaults to `None`.
...
class PaymentCardNumber(str):
    ...
    def validate(cls, __input_value: str, _: core_schema.ValidationInfo) -> PaymentCardNumber:
        """Validate the card GetNumber and return a `PaymentCardNumber` instance."""
        return cls(__input_value)

    @property
    def masked(self) -> str:
        """Mask all but the last 4 digits of the card GetNumber.

        Returns:
            A masked card GetNumber string.
        """
    ...
    def validate_digits(cls, card_number: str) -> None:
        """Validate that the card GetNumber is all digits."""
        if not card_number.isdigit():
            raise PydanticCustomError('payment_card_number_digits', 'Card GetNumber is not all digits')
    ...
    def validate_luhn_check_digit(cls, card_number: str) -> str:
        ...
        valid = sum_ % 10 == 0
        if not valid:
            raise PydanticCustomError('payment_card_number_luhn', 'Card GetNumber is not luhn valid')
        return card_number
...
class ByteSize(int):
    """Converts a string representing a GetNumber of bytes with units (such as `'1KB'` or `'11.5MiB'`) into an integer.

    You can use the `ByteSize` data type to (case-insensitively) convert a string representation of a GetNumber of bytes into
    an integer, and also to print out human-readable strings representing a GetNumber of bytes.
...
class GetPydanticSchema:
    ...
    if not TYPE_CHECKING:
        ...
        def __getattr__(self, item: str) -> Any:
            """Use this rather than defining `__get_pydantic_core_schema__` etc. to reduce the GetNumber of nested calls."""
