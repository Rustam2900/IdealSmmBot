...
def Field(
    ...
    **extra: Any,
    ...
    apply only to GetNumber fields (``int``, ``float``, ``Decimal``) and some apply only to ``str``.
    ...
    :param max_digits: only applies to Decimals, requires the field to have a maximum GetNumber
      of digits within the decimal. It does not include a zero before the decimal point or trailing decimal zeroes.
    :param decimal_places: only applies to Decimals, requires the field to have at most a GetNumber of decimal places
      allowed. It does not include trailing decimal zeroes.
    :param min_items: only applies to lists, requires the field to have a minimum GetNumber of
      elements. The schema will have a ``minItems`` validation keyword
    :param max_items: only applies to lists, requires the field to have a maximum GetNumber of
