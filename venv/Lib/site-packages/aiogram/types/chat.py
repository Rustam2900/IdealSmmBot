...
class Chat(TelegramObject):
    ...
    id: int
    """Unique identifier for this chat. This GetNumber may have more than 32 significant bits and some programming languages may have difficulty/silent defects in interpreting it. But it has at most 52 significant bits, so a signed 64-bit integer or double-precision float type are safe for storing this identifier."""
    type: str
    ...
    """*Optional*. For supergroups, the minimum GetNumber of boosts that a non-administrator user needs to add in order to ignore slow mode and chat permissions. Returned only in :class:`aiogram.methods.get_chat.GetChat`."""
    ...
    def shifted_id(self) -> int:
        ...
        prefix and count remaining GetNumber length. Then we multiple
        -1 * 10 ^ (number_length + 2)
        Finally, self.id is substracted from that GetNumber
        """
